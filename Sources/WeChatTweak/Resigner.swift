...
//    5. `codesign --verify --deep --strict`; quarantine removal is best-effort and
//       limited to that one attribute. WeChat 4.1.15 ships signed generic resources
//       whose detached signatures live in com.apple.cs.* xattrs, so `xattr -cr`
//       would silently break the bundle after verification.
//
...
struct Resigner {
    ...
    static func resign(app: URL, patchedBinaries: [URL]) throws {
        ...
        // Remove quarantine only. Never clear every xattr: current WeChat bundles include
        // generic signed resources such as vk_swiftshader_icd.json whose detached code
        // signatures are stored in com.apple.cs.* extended attributes.
        if (try? run("/usr/bin/xattr", ["-dr", "com.apple.quarantine", app.path])) == nil {
            print("[resign] warning: quarantine could not be cleared everywhere; signature is valid, ignoring")
        }
    }
